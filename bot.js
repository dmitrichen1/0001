...
- Отверстия под смеситель НЕ спрашивай отдельно — выводи из типа мойки: мойка под столешницу (нижняя) → для расчёта закладывай 2 отверстия как базу (больше — не меньше, это запас по прибыли; если клиент назвал фильтр/диспенсер — считай больше); мойка верхнего монтажа (накладная) → отверстия в камне не нужны, смеситель ставится в мойку
